...
export default function ProductosModal() {
    ...
    const calcularPrecio = (tipo, tamano, cobertura, cantidad) => {
        ...
        else if (tipo === 'ENVIO') unitario = getPrice(p.otros.envio);
                    ...
                    <div className="flex-1 flex overflow-hidden mr-1">
                        ...
                        <div className="w-1/2 flex flex-col bg-white dark:bg-slate-800">
                            ...
                            <div className="p-5 space-y-6 overflow-y-auto">
                                ...
                                <div className="grid grid-cols-2 gap-3"><button onClick={() => setServicioForm({ ...servicioForm, tipo: 'WIFI' })} className={`p-3 rounded-lg border text-sm font-bold flex flex-col items-center gap-2 transition-all hover:scale-105 ${servicioForm.tipo === 'WIFI' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}><div className={servicioForm.tipo === 'WIFI' ? 'text-white' : 'text-emerald-500'}><Wifi size={20}/></div>WiFi (${getPrice(config.precios.otros.wifi)})</button><button onClick={() => setServicioForm({ ...servicioForm, tipo: 'ASESORIA' })} className={`p-3 rounded-lg border text-sm font-bold flex flex-col items-center gap-2 transition-all hover:scale-105 ${servicioForm.tipo === 'ASESORIA' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}><div className={servicioForm.tipo === 'ASESORIA' ? 'text-white' : 'text-amber-500'}><HandHelping size={20}/></div>Asesoría (${getPrice(config.precios.otros.asesoria)})</button></div>
