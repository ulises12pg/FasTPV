...
export default function ProductosModal() {
    ...
    const calcularPrecio = (tipo, tamano, cobertura, cantidad) => {
        ...
        } else if (tipo === 'ESCANEO') unitario = tamano === 'OFICIO' ? (p.otros.escaneo?.oficio || 0) : (p.otros.escaneo?.carta || 0);
        else if (tipo === 'ENVIO') unitario = getPrice(p.otros.envio) || 0;
        else if (p.otros[tipoKey] !== undefined) unitario = getPrice(p.otros[tipoKey]) || 0;
                    ...
                    <div className="flex-1 flex overflow-hidden mr-1">
                        ...
                        <div className="w-1/2 flex flex-col bg-white dark:bg-slate-800">
                            ...
                            <div className="p-5 space-y-6 overflow-y-auto">
                                <div><label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-2">1. Selecciona Servicio</label><div className="grid grid-cols-2 gap-3">{[{ id: 'IMPRESION_BN', label: 'Impresión B/N', icon: <FileText size={20}/>, color: 'text-slate-600' }, { id: 'IMPRESION_COLOR', label: 'Impresión Color', icon: <Printer size={20}/>, color: 'text-pink-500' }, { id: 'ESCANEO', label: 'Escaneo', icon: <ScanBarcode size={20}/>, color: 'text-blue-500' }, { id: 'ENVIO', label: 'Envío Digital', icon: <Bluetooth size={20}/>, color: 'text-indigo-500' }].filter(opt => opt.id !== 'ENVIO' || config.precios.otros.envio !== undefined).map(opt => (<button key={opt.id} onClick={() => setServicioForm({...servicioForm, tipo: opt.id})} className={`p-3 rounded-lg border text-sm font-bold flex flex-col items-center gap-2 transition-all hover:scale-105 ${servicioForm.tipo === opt.id ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg ring-2 ring-indigo-200' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 shadow-sm'}`}><div className={servicioForm.tipo === opt.id ? 'text-white' : opt.color}>{opt.icon}</div>{opt.label}</button>))}</div></div>
                                <div className="grid grid-cols-2 gap-3">{config.precios.otros.wifi !== undefined && (<button onClick={() => setServicioForm({ ...servicioForm, tipo: 'WIFI' })} className={`p-3 rounded-lg border text-sm font-bold flex flex-col items-center gap-2 transition-all hover:scale-105 ${servicioForm.tipo === 'WIFI' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}><div className={servicioForm.tipo === 'WIFI' ? 'text-white' : 'text-emerald-500'}><Wifi size={20}/></div>WiFi (${getPrice(config.precios.otros.wifi)})</button>)}{config.precios.otros.asesoria !== undefined && (<button onClick={() => setServicioForm({ ...servicioForm, tipo: 'ASESORIA' })} className={`p-3 rounded-lg border text-sm font-bold flex flex-col items-center gap-2 transition-all hover:scale-105 ${servicioForm.tipo === 'ASESORIA' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}><div className={servicioForm.tipo === 'ASESORIA' ? 'text-white' : 'text-amber-500'}><HandHelping size={20}/></div>Asesoría (${getPrice(config.precios.otros.asesoria)})</button>)}</div>
                                ...
                                {servicioForm.tipo !== 'ENVIO' && servicioForm.tipo !== 'WIFI' && servicioForm.tipo !== 'ASESORIA' && (() => { const activos = config.precios.extras.tamanosActivos || { carta: true, oficio: true, tabloide: true }; const tamanos = ['CARTA', 'OFICIO', 'TABLOIDE'].filter(t => activos[t.toLowerCase()] !== false).filter(t => !(servicioForm.tipo === 'ESCANEO' && t === 'TABLOIDE')); return tamanos.length > 1 ? (<div className="fade-anim delay-75"><label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-2">3. Tamaño de Papel</label><div className="flex bg-slate-100 dark:bg-slate-700 p-1 rounded-lg">{tamanos.map(tam => (<button key={tam} onClick={() => setServicioForm({...servicioForm, tamano: tam})} className={`flex-1 py-2 text-xs rounded-md font-bold transition-all ${servicioForm.tamano === tam ? 'bg-white dark:bg-slate-600 text-indigo-600 dark:text-indigo-300 shadow-sm transform scale-105' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}>{tam}</button>))}</div></div>) : null; })()}
